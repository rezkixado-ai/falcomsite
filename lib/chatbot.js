...
async function sendMessage(req, res) {
  ...
  const waNumber = await getSetting('wa_admin_number', '6281234567890');
  ...
  if (shouldEscalate) {
    const waText = encodeURIComponent(`Halo Falcom, saya butuh bantuan terkait: ${message}`);
    ...
    return res.json({ reply, escalate: true, wa_link: `https://wa.me/${waNumber}?text=${waText}` });
  }
